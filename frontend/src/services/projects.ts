...
export function batchUpdateProjectStatus(baseUrl: string, token: string, tenantId: number, projectIds: number[], projectStatus: string) { return fetchJson<ApiResponse<{ updated: number }>>(`${baseUrl}/api/v1/projects/batch-status`, { method: 'POST', headers: { ...authHeaders(token, tenantId), 'Content-Type': 'application/json' }, body: JSON.stringify({ project_ids: projectIds, project_status: projectStatus }) }) }
export function loadProjectRiskRules(baseUrl: string, token: string, tenantId: number) { return fetchJson<ApiResponse<ProjectRiskRule[]>>(`${baseUrl}/api/v1/projects/risk-rules`, { headers: authHeaders(token, tenantId) }) }
export function updateProjectRiskRule(baseUrl: string, token: string, tenantId: number, ruleCode: string, payload: Record<string, unknown>) { return fetchJson<ApiResponse<ProjectRiskRule>>(`${baseUrl}/api/v1/projects/risk-rules/${encodeURIComponent(ruleCode)}`, { method: 'PUT', headers: { ...authHeaders(token, tenantId), 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }) }
