...
const receivablePageSize = ref(20)
...
type PagedList = 'accounts' | 'transactions' | 'receivables' | 'projects' | 'matching' | 'exceptions' | 'reports'

async function changePage(list: PagedList, page: number) {
  if (page < 1) return
  if (list === 'accounts' && page <= totalPages(totalAccounts.value, accountPageSize.value)) { accountPage.value = page; await loadAccountPage() }
  if (list === 'transactions' && page <= totalPages(totalTransactions.value, transactionPageSize.value)) { transactionPage.value = page; await loadTransactionPage() }
  if (list === 'receivables' && page <= totalPages(totalReceivables.value, receivablePageSize.value)) { receivablePage.value = page; await loadReceivablePage() }
  if (list === 'projects' && page <= totalPages(totalProjects.value, projectPageSize.value)) { projectPage.value = page; await loadProjectPage() }
  if (list === 'matching' && page <= totalPages(totalMatchResults.value, matchPageSize.value)) { matchPage.value = page; await loadMatchPage() }
  if (list === 'exceptions' && page <= totalPages(totalExceptions.value, exceptionPageSize.value)) { exceptionPage.value = page; await loadExceptionPage() }
  if (list === 'reports' && page <= totalPages(totalReports.value, reportPageSize.value)) { reportPage.value = page; await loadReportTasks() }
}

async function changePageSize(list: PagedList, value: string) {
  const pageSize = Number(value)
  if (![20, 50, 100].includes(pageSize)) return
  if (list === 'accounts') { accountPageSize.value = pageSize; accountPage.value = 1; await loadAccountPage() }
  if (list === 'transactions') { transactionPageSize.value = pageSize; transactionPage.value = 1; await loadTransactionPage() }
  if (list === 'receivables') { receivablePageSize.value = pageSize; receivablePage.value = 1; await loadReceivablePage() }
  if (list === 'projects') { projectPageSize.value = pageSize; projectPage.value = 1; await loadProjectPage() }
  if (list === 'matching') { matchPageSize.value = pageSize; matchPage.value = 1; await loadMatchPage() }
  if (list === 'exceptions') { exceptionPageSize.value = pageSize; exceptionPage.value = 1; await loadExceptionPage() }
  if (list === 'reports') { reportPageSize.value = pageSize; reportPage.value = 1; await loadReportTasks() }
}

async function loadAccountPage() {
  if (!user.value || !token.value) return
  const result = await loadAccounts(backendBase, token.value, user.value.tenant_id, accountPage.value, accountPageSize.value)
  accounts.value = result.data.items
  totalAccounts.value = result.data.total
...
async function loadTransactionPage() {
  if (!user.value || !token.value) return
  const result = await loadTransactions(backendBase, token.value, user.value.tenant_id, transactionPage.value, transactionPageSize.value)
  transactions.value = result.data.items
  totalTransactions.value = result.data.total
}

async function loadReceivablePage() {
  if (!user.value || !token.value) return
  const result = await loadReceivables(backendBase, token.value, user.value.tenant_id, receivablePage.value, receivablePageSize.value)
  receivables.value = result.data.items
  totalReceivables.value = result.data.total
}

async function loadProjectPage() {
  if (!user.value || !token.value) return
  const result = await loadProjects(backendBase, token.value, user.value.tenant_id, projectPage.value, projectPageSize.value)
  projects.value = result.data.items
  totalProjects.value = result.data.total
}

async function loadMatchPage() {
  if (!user.value || !token.value) return
  const result = await loadMatchResults(backendBase, token.value, user.value.tenant_id, matchPage.value, matchPageSize.value)
  matchResults.value = result.data.items
  totalMatchResults.value = result.data.total
}

async function loadExceptionPage() {
  if (!user.value || !token.value) return
  const result = await loadExceptions(backendBase, token.value, user.value.tenant_id, exceptionPage.value, exceptionPageSize.value)
  exceptions.value = result.data.items
  totalExceptions.value = result.data.total
}
...
async function loadReportTasks() {
  if (!user.value || !token.value) return
  const result = await loadReports(backendBase, token.value, user.value.tenant_id, reportPage.value, reportPageSize.value)
...
async function loadWorkspace() {
  ...
  try {
    const [overviewResult, accountResult, transactionResult, contractResult, receivableResult, matchResult, exceptionResult, projectResult, riskRuleResult] = await Promise.all([
      ...
      loadAccounts(backendBase, token.value, user.value.tenant_id, accountPage.value, accountPageSize.value),
      loadTransactions(backendBase, token.value, user.value.tenant_id, transactionPage.value, transactionPageSize.value),
      loadContracts(backendBase, token.value, user.value.tenant_id),
      loadReceivables(backendBase, token.value, user.value.tenant_id, receivablePage.value, receivablePageSize.value),
      loadMatchResults(backendBase, token.value, user.value.tenant_id, matchPage.value, matchPageSize.value),
      loadExceptions(backendBase, token.value, user.value.tenant_id, exceptionPage.value, exceptionPageSize.value),
      loadProjects(backendBase, token.value, user.value.tenant_id, projectPage.value, projectPageSize.value),
      loadProjectRiskRules(backendBase, token.value, user.value.tenant_id),
    ])
    overview.value = overviewResult.data
    accounts.value = accountResult.data.items
    totalAccounts.value = accountResult.data.total
    ...
    totalReceivables.value = receivableResult.data.total
    matchResults.value = matchResult.data.items
    totalMatchResults.value = matchResult.data.total
    exceptions.value = exceptionResult.data.items
    totalExceptions.value = exceptionResult.data.total
    projects.value = projectResult.data.items
    totalProjects.value = projectResult.data.total
...
<template>
  ...
  <main v-else class="app-shell">
    ...
    <template v-if="activeView === 'dashboard'">
      ...
      <section class="panel"><div class="section-heading"><h2>账户概览</h2><span class="meta">共 {{ totalAccounts }} 个账户</span></div><div class="account-grid"><div v-for="account in accounts" :key="account.id" class="account-row"><div><strong>{{ account.bank_name }} · {{ account.account_name }}</strong><p class="meta">账号后四位 {{ account.account_no_last4 }} · {{ account.status }}</p></div><strong>{{ formatCurrency(account.current_balance) }}</strong></div></div></section>
    </template>
    ...
    <template v-else>
      <section v-if="activeView === 'accounts'" class="grid account-management-layout">
        ...
        <article class="panel table-panel"><div class="section-heading"><div><h2>账户盘点</h2><span class="meta">共 {{ totalAccounts }} 个账户</span></div><button class="primary-button" :disabled="accountLoading" type="button" @click="runIdleScan">盘点闲置账户</button></div><div v-if="accounts.length" class="table-scroll"><table><thead><tr><th>银行/账户</th><th>后四位</th><th>余额</th><th>状态</th><th>最近动账</th><th>闲置级别</th><th>操作</th></tr></thead><tbody><tr v-for="account in accounts" :key="account.id"><td>{{ account.bank_name }}<br />{{ account.account_name }}</td><td>{{ account.account_no_last4 }}</td><td>{{ formatCurrency(account.current_balance) }}</td><td><span class="pill">{{ account.status }}</span></td><td>{{ account.last_transaction_at || '暂无流水' }}<br /><span v-if="account.idle_days !== null" class="meta">{{ account.idle_days }} 天</span></td><td><span class="pill" :class="account.idle_level === 'normal' ? '' : 'risk-medium'">{{ idleLabel[account.idle_level || 'normal'] }}</span></td><td><div class="action-group"><button class="text-button" type="button" @click="editAccount(account)">编辑</button><button v-if="account.status !== 'closed'" class="small-danger-button" type="button" @click="closeAccount(account)">标记销户</button></div></td></tr></tbody></table></div><p v-else class="empty-state">暂无银行账户。</p><div v-if="totalAccounts" class="pagination-controls"><span class="pagination-summary">第 {{ accountPage }} / {{ totalPages(totalAccounts, accountPageSize) }} 页，共 {{ totalAccounts }} 条</span><label>每页<select :value="accountPageSize" @change="changePageSize('accounts', ($event.target as HTMLSelectElement).value)"><option :value="20">20</option><option :value="50">50</option><option :value="100">100</option></select></label><div class="pagination-actions"><button class="ghost-button" :disabled="accountPage <= 1" type="button" @click="changePage('accounts', accountPage - 1)">上一页</button><button class="ghost-button" :disabled="accountPage >= totalPages(totalAccounts, accountPageSize)" type="button" @click="changePage('accounts', accountPage + 1)">下一页</button></div></div></article>
      </section>
      <section v-if="activeView === 'receivables'" class="grid receivable-layout">
        ...
        <article class="panel table-panel"><div class="section-heading"><h2>应收计划</h2><span class="meta">共 {{ totalReceivables }} 个节点</span></div><div v-if="receivables.length" class="table-scroll"><table><thead><tr><th>合同</th><th>客户</th><th>节点</th><th>应收日期</th><th>计划金额</th><th>已收金额</th><th>状态</th><th>追溯</th></tr></thead><tbody><tr v-for="item in receivables" :key="item.id"><td>{{ item.contract_no }}<br />{{ item.contract_name }}</td><td>{{ item.customer_name }}</td><td>{{ item.node_name }}</td><td>{{ item.due_date }}</td><td>{{ formatCurrency(item.plan_amount) }}</td><td>{{ formatCurrency(item.paid_amount) }}</td><td><span class="pill">{{ item.status }}</span></td><td><button class="text-button" type="button" @click="showDetail(`合同 ${item.contract_no}`, () => loadContractDetail(backendBase, token, user!.tenant_id, item.contract_id))">详情</button></td></tr></tbody></table></div><p v-else class="empty-state">暂无应收计划，请先导入合同 CSV。</p><div v-if="totalReceivables" class="pagination-controls"><span class="pagination-summary">第 {{ receivablePage }} / {{ totalPages(totalReceivables, receivablePageSize) }} 页，共 {{ totalReceivables }} 条</span><label>每页<select :value="receivablePageSize" @change="changePageSize('receivables', ($event.target as HTMLSelectElement).value)"><option :value="20">20</option><option :value="50">50</option><option :value="100">100</option></select></label><div class="pagination-actions"><button class="ghost-button" :disabled="receivablePage <= 1" type="button" @click="changePage('receivables', receivablePage - 1)">上一页</button><button class="ghost-button" :disabled="receivablePage >= totalPages(totalReceivables, receivablePageSize)" type="button" @click="changePage('receivables', receivablePage + 1)">下一页</button></div></div></article>
      </section>
      <section v-else-if="activeView === 'projects'" class="panel table-panel">
        <div class="section-heading"><div><h2>项目资金与风险</h2><span class="meta">共 {{ totalProjects }} 个项目</span></div><div class="action-group"><button class="small-primary-button" :disabled="!selectedProjectIds.length" type="button" @click="batchProjectStatus('active')">批量启用</button><button class="small-primary-button" :disabled="!selectedProjectIds.length" type="button" @click="batchProjectStatus('completed')">批量完成</button><button class="ghost-button" type="button" @click="loadProjectPage">刷新</button></div></div>
      ...
      <section v-else-if="activeView === 'matching'" class="panel table-panel">
        <div class="section-heading"><div><h2>匹配结果</h2><span class="meta">共 {{ totalMatchResults }} 条</span></div><span class="meta">待确认结果按匹配组整体处理</span></div>
      ...
      <section v-else-if="activeView === 'reports'" class="reports-page">
        ...
        <section class="grid reports-layout">
          ...
          <article class="panel table-panel"><div class="section-heading"><div><h2>报表任务</h2><span class="meta">共 {{ totalReports }} 条</span></div><button class="ghost-button" type="button" @click="loadReportTasks">刷新</button></div><div v-if="reports.length" class="table-scroll"><table><thead><tr><th>报表类型</th><th>统计范围</th><th>状态</th><th>文件</th><th>生成时间</th><th>操作</th></tr></thead><tbody><tr v-for="report in reports" :key="report.id" :class="{ 'selected-row': selectedReport?.id === report.id }"><td>{{ report.report_type === 'health' ? '资金体检' : report.report_type === 'monthly' ? '月报' : '日报' }}</td><td>{{ report.date_from || '-' }} 至 {{ report.date_to || '-' }}</td><td><span class="pill">{{ report.status }}</span></td><td>{{ report.file_name || '-' }}</td><td>{{ report.created_at }}</td><td><div class="action-group"><button class="text-button" type="button" @click="selectReport(report)">查看详情</button><button v-if="report.status === 'success'" class="text-button" type="button" @click="downloadReportFile(report)">下载 CSV</button></div></td></tr></tbody></table></div><p v-else class="empty-state">暂无报表任务，请先生成报表。</p><div v-if="totalReports" class="pagination-controls"><span class="pagination-summary">第 {{ reportPage }} / {{ totalPages(totalReports, reportPageSize) }} 页，共 {{ totalReports }} 条</span><label>每页<select :value="reportPageSize" @change="changePageSize('reports', ($event.target as HTMLSelectElement).value)"><option :value="20">20</option><option :value="50">50</option><option :value="100">100</option></select></label><div class="pagination-actions"><button class="ghost-button" :disabled="reportPage <= 1" type="button" @click="changePage('reports', reportPage - 1)">上一页</button><button class="ghost-button" :disabled="reportPage >= totalPages(totalReports, reportPageSize)" type="button" @click="changePage('reports', reportPage + 1)">下一页</button></div></div></article>
        </section>
