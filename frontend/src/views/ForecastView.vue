...
const forecast = ref<ForecastDetail>({
  job: null,
  results: [],
...
async function activate(version: string) {
  ...
  try {
    ...
    models.value = models.value.map((model) => ({
      ...model,
      status: model.version === updated.version ? 'active' : 'inactive',
    }))
...
<template>
  <section class="page-shell">
    <header class="hero-band">
      <div>
        <p class="eyebrow">现金预测</p>
        <h2>{{ route.params.jobId ? `预测任务 #${route.params.jobId}` : '现金流预测工作台' }}</h2>
        <p class="lead">结合历史净现金流和未来应收，查看预测余额、风险解释和评估偏差。</p>
      </div>
      <RouterLink v-if="route.params.jobId" class="ghost-button" to="/forecast"
        >返回预测工作台</RouterLink
      >
    </header>
    ...
    <section class="grid forecast-layout">
      <article class="panel workflow-panel">
        ...
        <section class="detail-section">
          ...
          <div v-for="model in models" :key="model.version" class="mini-row">
            ...
            >
              启用</button
            ><span v-else class="pill">当前启用</span>
          </div>
        </section>
      </article>
      <article class="panel table-panel">
        <div class="section-heading">
          ...
          <RouterLink
            v-if="forecast.job && !route.params.jobId"
            class="text-button"
            :to="`/forecast/jobs/${forecast.job.id}`"
            >打开任务详情</RouterLink
          >
        </div>
        <div v-if="forecast.job" class="summary-grid">
          <div>
            <span>预测区间</span
            ><strong
              >{{ forecast.job.input_start_date || '-' }} 至
              {{ forecast.job.input_end_date || '-' }}</strong
            >
          </div>
          <div>
            <span>模型</span
          ...
          <div>
            <span>尝试次数</span><strong>{{ forecast.job.attempt_count }}</strong>
          </div>
          <div>
            <span>完成时间</span><strong>{{ forecast.job.finished_at || '-' }}</strong>
          </div>
        </div>
        <p v-if="forecast.job?.error_message" class="error-banner">
          {{ forecast.job.error_message }}
        </p>
        <div v-if="forecast.results.length" class="table-scroll">
          <table>
            <thead>
              <tr>
                <th>日期</th>
            ...
            <tbody>
              <tr v-for="item in forecast.results" :key="item.id">
                <td>{{ item.forecast_date }}</td>
                <td :class="Number(item.forecast_amount) < 0 ? 'expense-amount' : 'income-amount'">
                  {{ formatCurrency(item.forecast_amount) }}
                </td>
                <td>{{ formatCurrency(item.expected_receivable) }}</td>
                <td>{{ formatCurrency(item.projected_balance) }}</td>
                <td>
                  {{ item.actual_amount !== null ? formatCurrency(item.actual_amount) : '-' }}
                </td>
                <td>
                  {{ item.deviation_amount !== null ? formatCurrency(item.deviation_amount) : '-' }}
                </td>
                <td>
        ...
        <p v-else class="empty-state">暂无预测结果，请先生成预测。</p>
        <div v-if="forecast.job" class="forecast-evaluation">
          <strong>评估点数 {{ forecast.evaluation.evaluated_points }}</strong
          ><span>MAE {{ formatCurrency(forecast.evaluation.mae) }}</span
          ><span>RMSE {{ formatCurrency(forecast.evaluation.rmse) }}</span
          ><span>平均偏差 {{ formatCurrency(forecast.evaluation.mean_deviation) }}</span>
        </div>
      </article>
    </section>
  </section>
